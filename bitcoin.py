...
def disimilarity(wt, X, Xi):
    a = X.index != u'Yi'
    b = Xi.index != u'Yi'
    mean_a = X.loc[a].mean()
    mean_b = Xi.loc[b].mean()
    std_a = X.loc[a].std(ddof=0)
    std_b = Xi.loc[b].std(ddof=0)
    return wt * ((((X.loc[a] - mean_a) / std_a) * ((Xi.loc[b] - mean_b) / std_b)).sum()) / Xi.loc[b].size
...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0, len(Xi.index)):
        numerator += Xi.iloc[i, -1] * math.exp(disimilarity(wt, X, Xi.iloc[i]))
        denominator += math.exp(disimilarity(wt, X, Xi.iloc[i]))
